#!/bin/bash
ddcfg="$HOME/.csapp/deploy-stage.cfg";
if [[ -f "$ddcfg" ]];
	then source $ddcfg; 
...
function process_main_job (){
	...
	queue_no=`curl --insecure -s -i --netrc -X POST --user "$user:$pass" "$jenkins/view/$app%20pipeline/job/$job_name/buildWithParameters?branch=$branch&destEnv=stage&forceWhole=yes" | perl -ne "print /Location:.http.*\/(.*)\//"`;
	queue_url="$jenkins/queue/item/$queue_no/api/json";
...
if [ "$switch" == "--just-deploy" ]; then
	...
	echo "Will build and then deploy $branch to STAGE host, details below"
	echo "--------------------------------";
	git log -1 $branch
	echo "--------------------------------";
